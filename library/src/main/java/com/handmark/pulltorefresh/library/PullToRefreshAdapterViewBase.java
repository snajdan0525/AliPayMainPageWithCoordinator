...
//					+ ". Total Items:" + totalItemCount);
	...
	private boolean isFirstItemVisible() {
		...
		if(mRefreshableView.getChildCount() > 0){
			int height = mRefreshableView.getChildAt(0).getHeight();
			if(height > 0 && (Math.abs(mRefreshableView.getY()) <= height))
				return false;
		}
//		View headerView = ((PullToRefreshListView)mRefreshableView).getHeaderSize()
